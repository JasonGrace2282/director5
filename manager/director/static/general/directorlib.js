...
Director.UI = class {
	static getComponentTemplate(name) {
		let componentTemplatesElement = document.getElementById("component-templates");
		let templates =  componentTemplatesElement.content.querySelectorAll(`.${name}`);

		if (templates.length === 0) {
			throw new DirectorLibError(`Couldn't find template named: ${name}`, this.constructor.name);
		}
    ...
    static createDialog(title, body, acceptText = "Continue", rejectText = "Nevermind") {
        return new Promise((resolve) => {
            const dialogElem = this.getComponentTemplate("nc-dialog").cloneNode(true);
	...
	static createSnackbar(message, type = Director.UI.Snackbar.INFO) {
		let snackbarElem = this.getComponentTemplate("nc-snackbar").cloneNode(true);
		...
		switch (type) {
			...
			case Director.UI.Snackbar.SUCCESS:
				...
				break;
			default:
				throw new DirectorLibError(`Invalid snackbar type passed: ${type}`, this.constructor.name)
		}
...
document.addEventListener("alpine:init", () => {
	Alpine.directive("tooltip", (el, { expression }, { evaluateLater, effect }) => {
		let getTooltipMessage = evaluateLater(expression)

		effect(() => {
			getTooltipMessage(tooltipMessage => {
				let tooltipElem = Director.UI.getComponentTemplate("tooltip").cloneNode(true);
				tooltipElem.firstElementChild.textContent = tooltipMessage;
				document.body.appendChild(tooltipElem);
				Alpine.initTree(tooltipElem);

				let instance = Popper.createPopper(el, tooltipElem, {
					placement: "top",
					modifiers: [
						{
							name: "offset",
