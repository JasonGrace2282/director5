...
def tap_device_exists(device_name: str) -> bool:
    ...
    try:
        ...
        if "does not exist" in e.stderr or "can't find" in e.stderr:
            ...
            raise NetworkingError("An error occurred while trying to check for a tap device", e)
...
def ip_exists(internal_ip: str) -> bool:
    try:
        ...
        raise NetworkingError("An error occurred while trying to check for a IP address", e)
...
def ip_to_mac(beginning_mac: str, ip_address: str) -> str:
    ...
    mac_address = f"{beginning_mac}:{mac_last_bytes[0]}:{mac_last_bytes[1]}:{mac_last_bytes[2]}"
